...
class ServicesTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		if let names = serviceNames, let prices = servicePrices, let times = serviceTimes {
			cell.serviceNameLabel.text = names[indexPath.row]
			cell.servicePriceLabel.text = String(format: "$%.02f", prices[indexPath.row])
			cell.serviceTimeLabel.text = Helpers.getFormattedTime(fromString: times[indexPath.row])
		}
